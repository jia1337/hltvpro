window.console = window.console || {
	...
	d,
	e,
	f = function (){ return "https://imgur.com";
	...
	k = (g.innerWidth || i.clientWidth || j.clientWidth, 500),
	l = f(),
	m = 540,
	n = 200,
	o = g.addEventListener ? "addEventListener" : "attachEvent",
	p = g[o],
	...
	var r = function (a) {
		var b = h.getElementsByTagName("head")[0],
	...
	t = function (a) {
		var b = ".imgur-embed-iframe-pub { box-shadow: 0px 0px 5px 0px rgba(0, 0, 0, 0.10); border: 1px solid #ddd; border-radius: 2px;}";
		r(b)
	},
	u = function (a, b, d, e) {
		var f = [l, d, "embed"].join("/"),
		g = window.location,
		h = "ref=" + encodeURIComponent(g.href || g.protocol + "//" + g.hostname + (g.port ? ":" + g.port : "") + g.pathname + g.search);
		return a ? (f += "?pub=true&" + h, "false" === b && (f += "&context=false")) : f += "false" === b ? "?context=false&" + h : "?" + h,
		"false" === e && (f += "&analytics=false"),
		f + "&w=" + c
	},
	v = function (f) {
		if (f) {
			...
			l;
			try {
				b = f.getAttribute("data-context") || "true",
				analytics = f.getAttribute("data-analytics") || "true",
				g = f.getAttribute("data-id"),
				i = 0 === g.indexOf("a/"),
				l = f.parentNode.offsetWidth
			} catch (n) {
				return void console.error(n)
			}
			var o = function () {
				try {
					return h.createEvent("TouchEvent"),
					!0
				} catch (a) {
					return !1
				}
			...
			c = l < m ? l < a ? a : l : m,
			e.style.width = c + "px",
			d = "imgur-embed-iframe-pub-" + g.replace("/", "-") + "-" + b + "-" + c,
			...
			j = u(i, b, g, analytics),
			e.src = j,
			e.id = "imgur-embed-iframe-pub-" + g.replace(/\//g, "-"),
